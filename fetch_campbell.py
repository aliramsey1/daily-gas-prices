...
def parse_body_prices(msg):
        """Extract prices from email body text - primary method for Campbell Oil emails."""
    body_text = ''
    ...
    prices = {}
    email_date = None
    # Extract effective date: "effective from MM-DD-YYYY HH:MM:SS"
    ...
    if m:
            ...
            pass
    # Parse product lines: "Regular 87 Eth <unit_price> <taxes> <total>"
    # Pattern: product_name followed by numbers
...
def parse_pdf_prices(pdf_bytes, debug=False):
    ...
    if not text or len(text.strip()) < 20:
                if debug: print('Campbell: PDF text too short or empty')
                            return None
    if debug:
                print(f'Campbell: PDF raw text (first 500):')
        print(repr(text[:500]))
                                                                                            ...
                                                                                            for check_line in lines[i:i+5]:
                                                                                                                ...
                                                                                                                if len(nums) >= 3:
                                                                                                                        ...
                                                                                                                        if 'reg' not in prices:
                                                                                                                                    ...
                                                                                                                                    if m: prices['reg'] = float(m.group(1)); print(f'Campbell: PDF reg={prices["reg"]} (dotall)')
                                                                                                                                            if 'prem' not in prices:
        m = re.search(r'Premium[^\n]*?Eth[^\n]*?(\d+\.\d{4,6})', text, re.IGNORECASE | re.DOTALL)
        if m: prices['prem'] = float(m.group(1)); print(f'Campbell: PDF prem={prices["prem"]} (dotall)')
                if 'dsl' not in prices:
        m = re.search(r'Diesel[^\n]*?Clr[^\n]*?(\d+\.\d{4,6})', text, re.IGNORECASE | re.DOTALL)
        if m: prices['dsl'] = float(m.group(1)); print(f'Campbell: PDF dsl={prices["dsl"]} (dotall)')
...
def process_campbell_emails():
    ...
    debug_done = False
    for msg in emails:
                # Try body parsing first (more reliable for Campbell Oil emails)
                prices, email_date = parse_body_prices(msg)
        # Fallback to PDF if body parsing failed
        if not prices:
